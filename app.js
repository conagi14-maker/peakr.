...
const _RARITY_ORDER = { SSR: 4, SR: 3, R: 2, N: 1 };

// レアリティ → 出現可能な天気
const _WEATHER_BY_RARITY = {
  N:   ['sunny', 'rain'],
  R:   ['night', 'rainbow'],
  SR:  ['rain', 'night', 'rainbow', 'shooting'],
  SSR: ['rainbow', 'shooting'],
};

const _WEATHER_LABELS = {
  sunny:    '☀️ 晴れ',
  rain:     '🌧️ 雨',
  night:    '🌌 夜空',
  rainbow:  '🌈 虹',
...
let _gachaAnimSkip = null; // スキップ用 resolve

function _pickGachaWeather(bestRarity) {
  const pool = _WEATHER_BY_RARITY[bestRarity] || ['sunny'];
  return pool[Math.floor(Math.random() * pool.length)];
}

function _playGachaClimb(results) {
  return new Promise(resolve => {
    const overlay = document.getElementById('gacha-anim-overlay');
    if (!overlay) { resolve(); return; }

    const best = results.reduce((a, b) => _RARITY_ORDER[a.rarity] >= _RARITY_ORDER[b.rarity] ? a : b);
    const weather = _pickGachaWeather(best.rarity);

    _setupGachaWeather(weather);

    const sky      = document.getElementById('ga-sky');
    const climber  = document.getElementById('ga-climber');
    const chest    = document.getElementById('ga-chest');
    const chestLight = document.getElementById('ga-chest-light');
    const flash    = document.getElementById('ga-flash');
    const label    = document.getElementById('ga-weather-label');

    // 初期化
    overlay.style.display = 'block';
    overlay.classList.remove('ga-fadeout');
    chest.classList.remove('ga-chest-open', 'ga-chest-shake');
    chestLight.style.display = 'none';
    flash.classList.remove('ga-flash-on');
    climber.style.animation = 'none';
    // リフロー強制でアニメーション再起動
    void climber.offsetWidth;
    climber.style.animation = '';

    if (label) { label.textContent = _WEATHER_LABELS[weather]; label.classList.add('ga-label-show'); }
    ...
    const finish = () => {
      ...
      _gachaAnimSkip = null;
      overlay.classList.add('ga-fadeout');
      setTimeout(() => {
        overlay.style.display = 'none';
        overlay.classList.remove('ga-fadeout');
        if (label) label.classList.remove('ga-label-show');
        resolve();
      }, 350);
    };
    _gachaAnimSkip = finish;

    // 登山者が頂上に到達（CSSアニメ 3.2s）
    timers.push(setTimeout(() => {
      chest.classList.add('ga-chest-shake');
    }, 3300));
    // 宝箱が開く
    timers.push(setTimeout(() => {
      chest.classList.remove('ga-chest-shake');
      chest.classList.add('ga-chest-open');
      chestLight.style.display = '';
      chestLight.style.background = `radial-gradient(circle, ${RARITY_COLORS[best.rarity]}ee 0%, ${RARITY_COLORS[best.rarity]}66 40%, transparent 70%)`;
    }, 3900));
    // フラッシュ
    timers.push(setTimeout(() => {
      flash.style.background = RARITY_COLORS[best.rarity];
      flash.classList.add('ga-flash-on');
    }, 4500));
    // 終了
...
function _skipGachaAnim() {
  if (_gachaAnimSkip) _gachaAnimSkip();
}

function _setupGachaWeather(weather) {
  const sky     = document.getElementById('ga-sky');
  const sun     = document.getElementById('ga-sun');
  const rainbow = document.getElementById('ga-rainbow');
  const wx      = document.getElementById('ga-weather');
  if (!sky) return;

  // 空のグラデーション
  const skyGradients = {
    sunny:    'linear-gradient(to bottom, #4facfe 0%, #a6e3ff 60%, #e0f7ff 100%)',
    rain:     'linear-gradient(to bottom, #6b7280 0%, #9ca3af 60%, #c7cdd6 100%)',
    night:    'linear-gradient(to bottom, #0b1026 0%, #1a2148 60%, #2d3561 100%)',
    rainbow:  'linear-gradient(to bottom, #5b9fd4 0%, #aee0f0 55%, #ffe9c7 100%)',
    shooting: 'linear-gradient(to bottom, #060818 0%, #131a3a 55%, #232a52 100%)',
  };
  sky.style.background = skyGradients[weather] || skyGradients.sunny;

  // 各要素の表示切替
  sun.style.display     = weather === 'sunny' ? '' : 'none';
  rainbow.style.display = weather === 'rainbow' ? '' : 'none';
  wx.innerHTML = '';

  if (weather === 'rain') {
    for (let i = 0; i < 60; i++) {
      const d = document.createElement('div');
      d.className = 'ga-raindrop';
      d.style.left = Math.random() * 100 + '%';
      d.style.animationDelay = Math.random() * 1.2 + 's';
      d.style.animationDuration = (0.5 + Math.random() * 0.4) + 's';
      wx.appendChild(d);
    }
  } else if (weather === 'night' || weather === 'shooting') {
    for (let i = 0; i < 50; i++) {
    ...
    if (weather === 'shooting') {
      for (let i = 0; i < 3; i++) {
        ...
        m.className = 'ga-shooting';
        m.style.top  = (5 + Math.random() * 30) + '%';
        m.style.left = (40 + Math.random() * 50) + '%';
        m.style.animationDelay = (0.8 + i * 1.1) + 's';
        wx.appendChild(m);
      }
    }
  }
