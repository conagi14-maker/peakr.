...
function goPage(id, btn) {
  ...
  if (id === 'dev-supporter') renderDevSupporterPage();
  ...
  if (id === 'supporter')          renderSupporterPage();
  if (id === 'supporter-settings') renderSupporterSettingsPage();
...
// ── サポーター管理ページ ──
async function renderDevSupporterPage() {
  const el = document.getElementById('dev-sp-list');
  if (!el) return;
  el.textContent = '読み込み中…';
  const list = await dbDevFetchAllSupporters();
  if (!list || !list.length) {
    el.innerHTML = '<p style="color:var(--text3);font-size:13px">データなし</p>';
    return;
  }
  el.innerHTML = list.map(p => `
    <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--border)">
      <div>
        <div style="font-size:13px;font-weight:700">${p.name || '—'} <span style="font-weight:400;color:var(--text3)">(${p.account_id})</span></div>
        <div style="font-size:11px;color:var(--text3)">Lv.${p.level} / ${_SP_LABELS[p.personality] || p.personality}</div>
      </div>
      <button class="btn-danger" style="font-size:12px;padding:6px 12px" onclick="devResetSupporter('${p.account_id}')">リセット</button>
    </div>`).join('');
}

async function devResetSupporter(accountId) {
  const aid = accountId || document.getElementById('dev-sp-reset-id')?.value.trim()
              || localStorage.getItem('trendy_account_id');
  if (!aid) { showToast('account_id が空です', 'error'); return; }
  if (!confirm(`「${aid}」のサポーターデータをリセットしますか？`)) return;
  const ok = await dbResetSupporter(aid);
  if (ok) {
    showToast('リセットしました', 'success');
    // 自分自身のサポーターなら状態もクリア
    if (aid === localStorage.getItem('trendy_account_id')) {
      _supporterProfile = null;
      _spSetupStep = 0;
    }
    renderDevSupporterPage(); // 一覧を更新
  } else {
...
// 🤖 サポーター
// ══════════════════════════════════════════
let _supporterProfile = null;
let _spSetupStep = 0;                                  // 0=未開始 1〜4=各ステップ
let _spSetupData = { avatar_url: null, name: '', user_nickname: '' };
...
async function renderSupporterPage() {
  const aid = localStorage.getItem('trendy_account_id');
  if (!aid) return;
  _supporterProfile = await dbGetSupporterProfile(aid);
  const el = document.getElementById('supporter-content');
  if (!el) return;
  if (!_supporterProfile) {
    _spSetupStep > 0 ? _renderSupporterStep(el) : _renderSupporterSetup(el);
  } else {
    _renderSupporterMain(el);
  }
}

/* ── アバター HTML（96px or 任意サイズ） */
function _spAvatarHTML(profile, size = 96) {
  if (profile?.avatar_url) {
    return `<img src="${profile.avatar_url}" style="width:${size}px;height:${size}px;border-radius:50%;object-fit:cover;display:block">`;
  }
  return `<div class="sp-avatar-default" style="width:${size}px;height:${size}px;font-size:${Math.round(size * 0.44)}px">?</div>`;
}

/* ── EXPバー HTML */
function _spExpBarHTML(level, exp) {
  const needed = level * 20;
  const pct = Math.min(100, Math.round(exp / needed * 100));
  return `
    <div class="sp-exp-wrap">
      <div class="sp-exp-labels">
        <span>Lv.<b>${level}</b></span>
        <span>${exp} / ${needed} EXP</span>
      </div>
      <div class="sp-exp-track"><div class="sp-exp-fill" style="width:${pct}%"></div></div>
    </div>`;
}

/* ── 初回セットアップ画面（ボタン1つ） */
function _renderSupporterSetup(el) {
  el.innerHTML = `
    <div class="sp-setup">
      <div class="sp-setup-emoji">🧹</div>
      <h2 class="sp-setup-title">サポーターを迎えよう</h2>
      <p class="sp-setup-desc">あなたをサポートしてくれるメイドです。</p>
      <button class="sp-create-btn" onclick="_startSupporterSetup()">メイドを迎える</button>
    </div>`;
}

/* ── オンボーディング開始 */
function _startSupporterSetup() {
  _spSetupStep = 1;
  _spSetupData = { avatar_url: null, name: '', user_nickname: '' };
  const el = document.getElementById('supporter-content');
  if (el) _renderSupporterStep(el);
}

/* ── ステップ振り分け */
function _renderSupporterStep(el) {
  if (!el) return;
  switch (_spSetupStep) {
    case 1: _renderSpStep1(el); break;
    case 2: _renderSpStep2(el); break;
    case 3: _renderSpStep3(el); break;
    case 4: _renderSpStep4(el); break;
  }
}

/* ── チャット用アバター HTML */
function _spChatAvHTML() {
  if (_spSetupData.avatar_url) {
    return `<img src="${_spSetupData.avatar_url}" class="sp-chat-av">`;
  }
  return `<div class="sp-chat-av sp-chat-av--ph">?</div>`;
}
...
/* ── Step 1：アバター設定 */
function _renderSpStep1(el) {
  const myName = myNickname || 'ご主人様';
  el.innerHTML = `
    <div class="sp-onboard">
      <div class="sp-chat-row">
        <div class="sp-chat-av sp-chat-av--ph">?</div>
        <div class="sp-chat-bubble">${_esc2(myName)}様、初めまして！<br>まず私の見た目を決めてください。</div>
      </div>
      <div class="sp-onboard-action">
        <div class="sp-av-zone" id="sp-av-zone" onclick="document.getElementById('sp-setup-av-file').click()">
          <div id="sp-av-zone-inner">
            <div class="sp-av-zone-icon">📷</div>
            <div class="sp-av-zone-text">タップして画像を選ぶ</div>
          </div>
        </div>
        <input type="file" id="sp-setup-av-file" accept="image/*" style="display:none" onchange="_spAvatarFileSelected(this)">
        <button class="sp-next-btn" id="sp-next-1" onclick="_spGoStep(2)" disabled>次へ →</button>
      </div>
    </div>`;
}

/* ── Step 2：名前設定 */
function _renderSpStep2(el) {
  el.innerHTML = `
    <div class="sp-onboard">
      <div class="sp-chat-row">
        ${_spChatAvHTML()}
        <div class="sp-chat-bubble">ありがとうございます。<br>次に私の名前を決めてください。</div>
      </div>
      <div class="sp-onboard-action">
        <input id="sp-setup-name" class="sp-form-input" type="text" maxlength="20" placeholder="名前を入力"
          oninput="document.getElementById('sp-next-2').disabled=!this.value.trim()">
        <button class="sp-next-btn" id="sp-next-2" onclick="_spGoStep(3)" disabled>次へ →</button>
      </div>
    </div>`;
}

/* ── Step 3：呼び方設定 */
function _renderSpStep3(el) {
  el.innerHTML = `
    <div class="sp-onboard">
      <div class="sp-chat-row">
        ${_spChatAvHTML()}
        <div class="sp-chat-bubble">${_esc2(_spSetupData.name)}…ありがとうございます。<br>ご主人様のことはなんとお呼びしたらよいでしょうか？</div>
      </div>
      <div class="sp-onboard-action">
        <input id="sp-setup-nick" class="sp-form-input" type="text" maxlength="20" placeholder="例：ご主人様、お嬢様"
          oninput="document.getElementById('sp-next-3').disabled=!this.value.trim()">
        <button class="sp-next-btn" id="sp-next-3" onclick="_spGoStep(4)" disabled>次へ →</button>
      </div>
    </div>`;
}

/* ── Step 4：完了・説明 */
function _renderSpStep4(el) {
  const nick = _esc2(_spSetupData.user_nickname);
  el.innerHTML = `
    <div class="sp-onboard">
      <div class="sp-chat-row">
        ${_spChatAvHTML()}
        <div class="sp-chat-bubble">
          これからよろしくお願いします、${nick}様！<br><br>
          <b>私にできること</b><br>
          🔍 ${nick}様の好みを学んで、ぴったりな投稿をお持ちします<br>
          ⬆️ お世話をしてもらうことで成長していきます<br>
          💎 投稿・いいねで +10pt ゲットできます<br>
          🎯 ポイントを使ってアクションができます
        </div>
      </div>
      <div class="sp-onboard-action">
        <button class="sp-create-btn" onclick="_spComplete()">さっそく始める →</button>
      </div>
    </div>`;
}

/* ── オンボーディング Step1：クロップモーダルを開く */
function _spAvatarFileSelected(input) {
  ...
  reader.onload = e => {
    ...
    _avCropState.rotation = 0;
    // クロップ確定時のコールバック
    _avCropCallback = (dataUrl) => {
      _spSetupData.avatar_url = dataUrl;
      const inner = document.getElementById('sp-av-zone-inner');
      if (inner) inner.innerHTML = `<img src="${dataUrl}" style="width:100px;height:100px;border-radius:50%;object-fit:cover">`;
      const btn = document.getElementById('sp-next-1');
      if (btn) btn.disabled = false;
    };
    openAvCrop();
  };
...
/* ── ステップ遷移 */
function _spGoStep(step) {
  if (step === 3) {
    const v = document.getElementById('sp-setup-name')?.value.trim();
    if (!v) return;
    _spSetupData.name = v;
  }
  if (step === 4) {
    const v = document.getElementById('sp-setup-nick')?.value.trim();
    if (!v) return;
    _spSetupData.user_nickname = v;
  }
  _spSetupStep = step;
  const el = document.getElementById('supporter-content');
  if (el) _renderSupporterStep(el);
...
/* ── 完了 → DB 保存 */
async function _spComplete() {
  const aid = localStorage.getItem('trendy_account_id');
  if (!aid) return;
  const { name, avatar_url, user_nickname } = _spSetupData;
  const ok = await dbUpsertSupporterProfile(aid, {
    name:          name || 'メイド',
    avatar_url:    avatar_url || null,
    user_nickname: user_nickname || 'ご主人様',
    personality:   'maid',
    level: 1, exp: 0, is_active: true,
  });
  if (!ok) { showToast('保存に失敗しました', 'error'); return; }
  await _initSupporterTastes(aid);
  _spSetupStep = 0;
  _supporterProfile = await dbGetSupporterProfile(aid);
  const el = document.getElementById('supporter-content');
  if (el) _renderSupporterMain(el);
  showToast(`${name || 'メイド'}を迎えました！`, 'success');
}

/* ── メイン画面（カード＋トグル＋情報） */
function _renderSupporterMain(el) {
  const p = _supporterProfile;
  const name = p.name || 'サポーター';
  ...
  el.innerHTML = `
    ...
    <div class="sp-card">
      ...
      <div class="sp-name">${_esc2(name)}</div>
      <div class="sp-personality-badge">${_SP_LABELS[p.personality] || p.personality}</div>
      ${_spExpBarHTML(p.level, p.exp)}
    </div>

    <!-- ON/OFF トグル -->
    <div class="sp-toggle-row">
      <div>
        <div class="sp-toggle-title">サポーターを使う</div>
        <div class="sp-toggle-sub">オフにすると活動を停止します</div>
      </div>
      <label class="sp-toggle-switch">
        <input type="checkbox" id="sp-active-chk" ${p.is_active ? 'checked' : ''} onchange="toggleSupporterActive(this.checked)">
        <span class="sp-toggle-slider"></span>
      </label>
    </div>

    <!-- 情報 -->
    <div class="sp-section sp-level-info">
      <div class="sp-level-row">
        <span>🫅 あなたの呼び方</span>
        <span>${_esc2(p.user_nickname || 'ご主人様')}様</span>
      </div>
      <div class="sp-level-row">
        <span>📅 利用開始</span>
      ...
      <div class="sp-level-row">
        <span>⬆️ 次のレベルまで</span>
        <span>${p.level * 20 - p.exp} EXP</span>
      </div>
      <p class="sp-level-note">${_spLevelNote(p.level)}</p>
    </div>
  `;
}

/* ── 設定ページ */
function renderSupporterSettingsPage() {
  const el = document.getElementById('supporter-settings-content');
  if (!el) return;
  if (!_supporterProfile) {
    // プロフィール未ロードなら取得してから描画
    const aid = localStorage.getItem('trendy_account_id');
    if (!aid) return;
    dbGetSupporterProfile(aid).then(p => {
      if (p) { _supporterProfile = p; renderSupporterSettingsPage(); }
    });
    return;
  }
  const p = _supporterProfile;
  const name = p.name || 'サポーター';
  const nick = p.user_nickname || 'ご主人様';

  el.innerHTML = `
    <!-- アイコン -->
    <div class="sp-section" style="text-align:center">
      <h3 class="sp-section-title">アイコン</h3>
      <div class="sp-avatar-wrap sp-settings-av" onclick="openSupporterAvatarPicker()" title="変更">
        ${_spAvatarHTML(p, 80)}
        <div class="sp-avatar-cam"><i class="ti ti-camera"></i></div>
      </div>
      <p style="font-size:12px;color:var(--text3);margin-top:8px">タップして変更</p>
      <input type="file" id="sp-avatar-file" accept="image/*" style="display:none" onchange="_onSupporterAvatarChange(this)">
    </div>

    <!-- 名前 -->
    <div class="sp-section">
      <h3 class="sp-section-title">名前</h3>
      <div style="display:flex;gap:8px">
        <input id="sp-name-edit" class="sp-form-input" type="text" maxlength="20" value="${_esc2(name)}">
        <button class="sp-save-btn" onclick="saveSupporterName()">保存</button>
      </div>
    </div>

    <!-- 呼び方 -->
    <div class="sp-section">
      <h3 class="sp-section-title">あなたの呼び方</h3>
      <p style="font-size:12px;color:var(--text2);margin-bottom:8px">${_esc2(name)}があなたをなんと呼ぶか設定します</p>
      <div style="display:flex;gap:8px">
        <input id="sp-nick-edit" class="sp-form-input" type="text" maxlength="20"
          value="${_esc2(nick)}" placeholder="例：ご主人様、お嬢様">
        <button class="sp-save-btn" onclick="saveSupporterNickname()">保存</button>
      </div>
    </div>
  `;
}
...
/* レベル説明テキスト */
function _spLevelNote(level) {
  if (level < 5)  return '📖 あなたの好みを学習中です。';
  if (level < 10) return '🔍 カテゴリ内の細かい好みを把握しています。';
  if (level < 20) return '✨ 見落としがちな投稿も拾えるようになってきました。';
  if (level < 30) return '🎯 投稿のタイミングまで考慮して探してくれます。';
  return '💎 直感的なおすすめができるようになりました。';
}

/* ── アクション群 */

async function createSupporter(type) {
  const aid = localStorage.getItem('trendy_account_id');
  if (!aid) return;
  const name = type === 'maid' ? 'メイド' : type === 'butler' ? '執事' : 'ペット';
  const ok = await dbUpsertSupporterProfile(aid, { name, personality: type, level: 1, exp: 0, is_active: true });
  if (!ok) { showToast('作成に失敗しました', 'error'); return; }
  await _initSupporterTastes(aid);
  _supporterProfile = await dbGetSupporterProfile(aid);
  const el = document.getElementById('supporter-content');
  if (el) _renderSupporterMain(el);
  showToast(`${name} を迎えました！`, 'success');
}

async function toggleSupporterActive(active) {
  const aid = localStorage.getItem('trendy_account_id');
  if (!aid) return;
  await dbUpsertSupporterProfile(aid, { is_active: active });
  if (_supporterProfile) _supporterProfile.is_active = active;
}

async function saveSupporterName() {
  const aid = localStorage.getItem('trendy_account_id');
  const name = document.getElementById('sp-name-edit')?.value.trim();
  if (!aid || !name) return;
  const ok = await dbUpsertSupporterProfile(aid, { name });
  if (!ok) { showToast('保存に失敗しました', 'error'); return; }
  if (_supporterProfile) _supporterProfile.name = name;
  showToast('名前を変更しました', 'success');
}

async function saveSupporterNickname() {
  const aid = localStorage.getItem('trendy_account_id');
  const nick = document.getElementById('sp-nick-edit')?.value.trim();
  if (!aid || !nick) return;
  const ok = await dbUpsertSupporterProfile(aid, { user_nickname: nick });
  if (!ok) { showToast('保存に失敗しました', 'error'); return; }
  if (_supporterProfile) _supporterProfile.user_nickname = nick;
  showToast('呼び方を変更しました', 'success');
}

async function saveSupporterPersonality() {
  const aid = localStorage.getItem('trendy_account_id');
  const personality = document.querySelector('input[name="sp-type-edit"]:checked')?.value;
  if (!aid || !personality) return;
  const ok = await dbUpsertSupporterProfile(aid, { personality });
  if (!ok) { showToast('保存に失敗しました', 'error'); return; }
  if (_supporterProfile) _supporterProfile.personality = personality;
  const badge = document.querySelector('.sp-personality-badge');
  if (badge) badge.textContent = _SP_LABELS[personality] || personality;
  showToast('性格タイプを変更しました', 'success');
}

function openSupporterAvatarPicker() {
  document.getElementById('sp-avatar-file')?.click();
}

/* ── プロフィール編集：アバター変更（クロップモーダル経由） */
function _onSupporterAvatarChange(input) {
  if (!input.files?.[0]) return;
  const reader = new FileReader();
  reader.onload = e => {
    _avCropState.imageData = e.target.result;
    ...
    _avCropState.offsetY = 0;
    _avCropState.rotation = 0;
    _avCropCallback = async (dataUrl) => {
      const aid = localStorage.getItem('trendy_account_id');
      if (!aid) return;
      const ok = await dbUpsertSupporterProfile(aid, { avatar_url: dataUrl });
      if (!ok) { showToast('アバターの保存に失敗しました', 'error'); return; }
      if (_supporterProfile) _supporterProfile.avatar_url = dataUrl;
      const wrap = document.querySelector('.sp-avatar-wrap');
      if (wrap) {
        const existing = wrap.querySelector('img, .sp-avatar-default');
        if (existing) existing.remove();
        const img = document.createElement('img');
        img.src = dataUrl;
        img.style.cssText = 'width:96px;height:96px;border-radius:50%;object-fit:cover;display:block';
        wrap.prepend(img);
      }
      showToast('アバターを変更しました', 'success');
    };
    openAvCrop();
  };
  reader.readAsDataURL(input.files[0]);
...
/** いいね履歴から初期の好み重みを生成 */
async function _initSupporterTastes(accountId) {
  const tagCount = {}, catCount = {};
  TWEETS.forEach((t, idx) => {
    if (!likedTweets.has(idx)) return;
    if (t.catId) catCount[t.catId] = (catCount[t.catId] || 0) + 1;
  ...
  const normalize = obj => {
    ...
    const out = {};
    for (const [k, v] of Object.entries(obj)) out[k] = Math.round(v / max * 100);
    return out;
  };
  await dbUpsertSupporterTastes(accountId, normalize(tagCount), normalize(catCount));
}
